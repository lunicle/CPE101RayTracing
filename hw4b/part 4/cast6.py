...
def cast_ray(ray, sphere_list, ambient_color,light):
    ...
    if result == []:
        ...
        light_dir = vm.normalize_vector(vm.difference_point(P_epsilon,light.pt))
        visible = vm.dot_vector(normal,light_dir)

        check1 = check_same_side(normal,light_dir)
        check2 = check_light_collision(P_epsilon,light_dir,sphere_list)
